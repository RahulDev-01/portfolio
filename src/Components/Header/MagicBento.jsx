...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      ...
      // Uniform glow across the entire navbar width: all cards get the same intensity
      const { proximity, fadeDistance } = calculateSpotlightValues(spotlightRadius);
      cards.forEach(card => {
        const cardElement = card;
        // Use full intensity so every card shows the same effect while hovering anywhere in the navbar
        updateCardGlowProperties(cardElement, e.clientX, e.clientY, 1, spotlightRadius);
      });
      ...
      // Keep spotlight visible uniformly when inside the navbar area
      gsap.to(spotlightRef.current, {
        opacity: 0.8,
        duration: 0.2,
        ease: 'power2.out'
                ...
                ref={el => {
                  ...
                  const handleClick = e => {
                    ...
                    ripple.style.cssText = `
                      ...
                      height: ${maxDistance * 2}px;
                      border-radius: 6px;                      background: radial-gradient(circle, rgba(${glowColor}, 0.4) 0%, rgba(${glowColor}, 0.2) 30%, transparent 70%);
                      left: ${x - maxDistance}px;
                      top: ${y - maxDistance}px;
                      pointer-events: none;
                      z-index: 1000;
